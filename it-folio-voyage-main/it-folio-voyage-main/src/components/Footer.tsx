
import React from 'react';

const Footer = () => {
  return (
    <footer className="bg-tech-dark py-12 text-white/70">
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-8 pb-8 border-b border-white/10">
          <div>
            <h3 className="text-xl font-bold text-white mb-4">IT<span className="text-tech-purple">Portfolio</span></h3>
            <p className="text-sm leading-relaxed">
              Focused on creating exceptional digital experiences and innovative solutions for the modern web.
            </p>
          </div>
          
          <div className="md:text-center">
            <h4 className="text-white font-medium mb-4">Quick Links</h4>
            <div className="flex flex-col space-y-2">
              <a href="#home" className="hover:text-tech-purple transition-colors duration-300">Home</a>
              <a href="#about" className="hover:text-tech-purple transition-colors duration-300">About</a>
              <a href="#skills" className="hover:text-tech-purple transition-colors duration-300">Skills</a>
              <a href="#projects" className="hover:text-tech-purple transition-colors duration-300">Projects</a>
              <a href="#contact" className="hover:text-tech-purple transition-colors duration-300">Contact</a>
            </div>
          </div>
          
          <div className="md:text-right">
            <h4 className="text-white font-medium mb-4">Contact Information</h4>
            <div className="flex flex-col space-y-2 text-sm">
              <p>Jakarta, Indonesia</p>
              <p>contact@itportfolio.com</p>
              <p>+1 (555) 123-4567</p>
            </div>
          </div>
        </div>
        
        <div className="flex flex-col md:flex-row items-center justify-between">
          <div className="mb-4 md:mb-0">
            <p className="text-sm">
              &copy; {new Date().getFullYear()} IT Portfolio. All rights reserved.
            </p>
          </div>
          <div className="flex space-x-6">
            <a 
              href="#" 
              className="hover:text-tech-purple transition-colors duration-300"
              aria-label="Github"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-github">
                <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"/>
                <path d="M9 18c-4.51 2-5-2-7-2"/>
              </svg>
            </a>
            <a 
              href="#" 
              className="hover:text-tech-purple transition-colors duration-300"
              aria-label="LinkedIn"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-linkedin">
                <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/>
                <rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/>
              </svg>
            </a>
            <a 
              href="#" 
              className="hover:text-tech-purple transition-colors duration-300"
              aria-label="Email"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-mail">
                <rect width="20" height="16" x="2" y="4" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
              </svg>
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
